...
class statbin:
    ...
    def plot(self, lbl=""):
        ...
        plt.fill_between(self.times(), self.mean() - self.std(), self.mean() + self.std(), facecolor='lightblue',
                         alpha=0.5)

    def plot2(self, fill_col="lightblue", alpha=0.5, **kwargs):
        plt.plot(self.times(), self.mean(), **kwargs)
        plt.fill_between(self.times(), self.mean() - self.std(), self.mean() + self.std(), facecolor=fill_col,
                         alpha=alpha)
